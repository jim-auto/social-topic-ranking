...
seedに上限をかけても前半テーマだけに偏らないよう、テーマごとに1語ずつラウンドロビンで展開する。
...
fallback分類では、検索語全体と辞書語の完全一致、または空白除去後の完全一致を部分一致より強く扱う。これにより `コンカフェ` が `カフェ` に、`ゲーム実況` が `ゲーム` に吸われすぎないようにする。
...
| segment | meaning |
| --- | --- |
| `男性寄り` | メンズ、男性、ひげ、AGA、車、バイク、FANZA、グラビアなど男性向け文脈が強い検索語。 |
...
`副業・稼ぐ` は性別を決め打ちしない。`アダルト・性` のうち、FANZA、AV、グラビアなど一部の明示語だけを男性寄りの弱いシグナルとして扱う。
...
サイトのRankingでは表示用の合算フィルタとして `20代以下` を持つ。これは `10代・学生` と `20代` を合わせた再集計で、CSVの `generation_segment` 自体は変更しない。
若年層カルチャーの検索語として、SNS、TikTok、Instagram、MBTI、推し活、韓国コスメ、カラコン、プリクラ、カラオケなどは20代文脈の弱いシグナルとして扱う。
