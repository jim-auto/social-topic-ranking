...
`--build-site` を付けると `site/` に静的ダッシュボードを生成します。Rankingでは `男女寄り` と `世代` のフィルタを選ぶと、検索ワード単位の `audience_segment` / `generation_segment` に合わせてテーマランキングを再集計します。

```bash
python -m src.main --input data/sample_trends.csv --build-site
```

フィルタ用の軽量データは `site/data/filter_records.json` に出力されます。CSVの全量は `classified_keywords.csv` に残します。
...
| value | meaning |
| --- | --- |
| `男性寄り` | `メンズ`, `男性`, `彼女`, `ひげ`, `AGA`, `車`, `バイク`, `FANZA`, `グラビア` などの語が強い検索語。 |
